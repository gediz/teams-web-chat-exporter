...
if (existsSync(twemojiSrc)) {
  ...
  // Sort so pack.json / index.json are byte-deterministic across machines:
  // readdirSync order is filesystem-dependent, which would otherwise make the
  // generated bytes (and their hashes in vendor-hashes.json) vary by host.
  const files = readdirSync(twemojiSrc).filter(f => f.endsWith('.svg')).sort();
  const pack = {};
