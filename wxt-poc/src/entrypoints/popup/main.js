...
async function persistOptions() {
    try {
        await storage.local.set({ [STORAGE_KEY]: collectOptions() });
    } catch (_) {
...
async function init() {
    ...
    try {
        const tab = await getActiveTeamsTab();
        currentTabId = tab.id;
        const status = await runtime.sendMessage({ type: "GET_EXPORT_STATUS", tabId: currentTabId });
        if (status?.active) {
